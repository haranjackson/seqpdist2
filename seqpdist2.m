function D = seqpdist2(seqs,varargin)
...
%
%     'Felsenstein'     -  An extension of the Jukes-Cantor model in which
%                          base frequencies are allowed to vary from 0.25.
%                          Frequencies can be computed from the input
%                          sequences or given by setting 'OPTARGS' to
%                          [gA gC gG gT].
...
%     'GTR'                The most general neutral, independent,
...
%
%  SEQPDIST(...,'USEPARALLEL',true) computes the pairwise alignments in
%  parallel on separate MATLAB workers. In order to execute the alignments
%  in parallel you must have the Parallel Computing Toolbox installed and a
%  Parallel Pool available. If a Parallel Pool does not exist then one is
%  automatically created when the auto-creation option is enabled in your
%  parallel preferences. Computation runs in serial mode without issuing
%  any warning otherwise. Defaults to false, or serial computation mode.
%
        ...
        elseif length(k)>1
            ...
            switch(k)
                case 1 % select method to compute distances
                    if isa(pval,'function_handle')
                        ...
                        distMethods = {'p-distance','jukes-cantor',...
                            'felsenstein','alignment-score','tajima-nei',...
                            'kimura','tamura','hasegawa','nei-tamura','gtr',...
                            'poisson','gamma'};
                ...
                case 12 % Use parallel
                    useParallel = bioinfoprivate.opttf(pval,okargs{k},mfilename);
            end
        end
    end
end
...
if (indelMethod == 's') && any(distMethod == 'tkmhnogfr')
...
%-% Felsenstein distance
function d = felsenstein(s1,s2,h,g)
% Felsenstein distance
%
% Reference: en.wikipedia.org/wiki/Models_of_DNA_evolution
%
p = sum( (s1~=s2) & h ) / sum(h);
b = 1 - (g(1)^2 + g(2)^2 + g(3)^2 + g(4)^2);
d = -b * log(max(eps,1-p/b));
...
%-% GTR distance
function d = gtrdistance(s1,s2,h,g)
% GTR distance
%
% Reference: http://www.ncbi.nlm.nih.gov/pmc/articles/PMC1274259/
X  = accumarray(double([nt2int(s1(h))',nt2int(s2(h))']),1,[4 4]);
numPairs = sum(X(:));
F = double(X)/numPairs;
PI = diag(g);
